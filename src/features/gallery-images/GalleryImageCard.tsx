...
import { useUpdateGalleryImage, useDeleteGalleryImage } from "./hook/galleryImageHooks";

interface GalleryImageCardProps {
  image: SerializedGalleryImage;
}
...
export default function GalleryImageCard({ image }: GalleryImageCardProps) {
  ...
  const { mutateAsync: updateGalleryImage } = useUpdateGalleryImage();
  const { mutateAsync: deleteGalleryImage } = useDeleteGalleryImage();
  ...
  const handleToggle = async () => {
    ...
    setError(null);
    try {
      await updateGalleryImage({ id, data: { isActive: !image.isActive } });
      router.refresh();
    } catch (err) {
  ...
  const handleOrderSave = async () => {
    ...
    setError(null);
    try {
      await updateGalleryImage({ id, data: { order: newOrder } });
      setEditingOrder(false);
      router.refresh();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Network error.");
